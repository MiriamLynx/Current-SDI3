...
public class RestClient {
	...
	private static ContactoServiceRest contactService;
	...
	public RestClient() {
		...
		contactService = ProxyFactory.create(ContactoServiceRest.class,
				"http://localhost:8280/sdi3-1/rest/", clientexecutor);
	}
	...
	private static void sendMail() {
		printHeader("Destinatarios disponibles para " + login);
		List<Contacto> contactos = contactService.findByLogin(login);
		printContacts(contactos);
		System.out
				.println("Introduzca los identificadores de los destinatarios separados por ','");
		List<Contacto> destinatarios = getRecipients(readLine());
		if (checkDestinatarios(destinatarios, contactos)) {
	...
	private static boolean checkDestinatarios(List<Contacto> destinatarios,
	...
	private static boolean find(Contacto destinatario, List<Contacto> contactos) {
		for (Contacto c : contactos) {
			if (destinatario.getId() == c.getId()) {
				return true;
			}
		}
		return false;
	...
	private static void printContacts(List<Contacto> contactos) {
		for (Contacto c : contactos) {
			System.out.println("\t" + "[ Identificador: " + c.getId() + ", "
					+ "Nombre: " + c.getNombre() + ", " + "Email: "
					+ c.getEmail() + " ]");
		}
		System.out.println();
	}
